    ...
    public class GameDBRepository(IDictionary<string, string> dbConnection) : GameRepository
    {
        ...
        public Game? Save(Game game)
        {
            try
            {
                ...
                {
                    command.CommandText =
                        "insert into \"Game\" (\"teamA\", \"teamB\", \"date\", \"price\", \"type\", \"seats\") " +
        ...
        public Game? Update(Game game)
        {
            ...
            using (var command = con.CreateCommand())
            {
                command.CommandText = "update \"Game\" set \"teamA\" = @teamA, \"teamB\" = @teamB, \"date\" = @date, " +
                ...
                paramId.ParameterName = "@id";
                paramId.Value = game.get_identitykey();
                command.Parameters.Add(paramId);

                command.ExecuteNonQuery();

                log.Info($"Game with id {game.get_identitykey()} updated successfully.");
            }
        ...
        public List<Game> findByTypeOrderedByDate(string type)
        {
            ...
            using (var command = con.CreateCommand())
            {
                command.CommandText = "SELECT * FROM \"Game\" WHERE \"type\" = @type ORDER BY \"date\" ASC";
