package ro.mpp2024.jdbc;

import ro.mpp2024.Game;
import ro.mpp2024.RepositoryGame;

import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;
import ro.mpp2024.Type;
        ...
        try (PreparedStatement preStmt = conn.prepareStatement("SELECT * FROM Games WHERE id = ?")) {
            ...
            try (ResultSet resultSet = preStmt.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    Game game = new Game(ID1, teamA, teamB, date, price, type, seats);
        ...
        try (PreparedStatement preStmt = con.prepareStatement("SELECT * FROM Games WHERE type = ? ORDER BY date ASC")) {
            ...
            try (ResultSet resultSet = preStmt.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    Game game = new Game(ID1, teamA, teamB, date, price, gameType, seats);
                    games.add(game);
                }
            }
        } catch (SQLException e) {
