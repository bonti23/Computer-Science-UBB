...
VectorDinamic<Type>::VectorDinamic(int cp) {
    if (cp < 1) {
        throw std::runtime_error("Capacitatea vectorului trebuie sa fie un numar pozitiv!");
    }
...
void VectorDinamic<Type>::adauga(int i, Type e) {
    if (i < 0 || i > n) {
        throw std::runtime_error("Nu s-a putut adauga elementul, pozitia este invalida!");
    }
    if (n == cp) {
        redim();
    }
...
Type VectorDinamic<Type>::modifica(int i, Type e) {
    if (i < 0 || i >= n) {
        throw std::runtime_error("Nu s-a putut realiza modificarea, pozitia este invalida!");
    }
    Type old = this->e[i];
    this->e[i] = e;
    return old;
...
Type VectorDinamic<Type>::sterge(int i) {
    if (i < 0 || i >= n) {
        throw std::runtime_error("Nu s-a putut sterge elementul, pozitia este invalida!");
    }
    Type old = e[i];
    for (int j = i; j < n; ++j) {
        e[j] = e[j + 1];
